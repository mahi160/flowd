...
<Card heading="Best hours" description="focus by hour-of-day">
  <div class="grid grid-cols-24 gap-[2px] h-20 items-end mt-1 mb-5 relative">
    {#each hourly as h (h.hour)}
      <div class="relative h-full flex items-end">
        <span
          class="block w-full min-h-[2px] rounded-t-[2px]"
          style:height="{h.intensity * 100}%"
          style:background="linear-gradient(180deg, var(--accent), color-mix(in oklch, var(--accent) 40%, transparent))"
        ></span>
        {#if h.hour % 4 === 0}
          <span
