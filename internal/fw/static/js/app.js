...
function ProjectBreakdown(DATA) {
  ...
  DATA.byProject.slice(0, 8).forEach(p => {
    const swatch = el('span', 'legend-swatch'); swatch.style.background = p.color;
    const fill   = el('span'); fill.style.width = `${(p.min / topMin) * 100}%`; fill.style.background = p.color;
    const bar    = el('div', 'lang-bar-mini', fill);
    const li     = el('li', null, swatch, el('span', 'lang-name', p.name), bar, el('span', 'tnum lang-val', p.min + 'm'));
...
function WrongPeriod(requested) {
  const main = el('main', 'dashboard');
  main.appendChild(el('div', 'card empty-state',
...
function Footer() {
  return el('footer', 'foot', 'flowd', el('span', 'dim', ' — local activity tracker · self-hosted'));
}
...
function render() {
  ...
  page.appendChild(Header(DATA, currentView, v => { currentView = v; render(); }));
