<script lang="ts">
  ...
  export let data: { generated: string; hasData: boolean; period: string };
  export let view: string;
  export let setView: (v: string) => void;
...
<header class="flex items-center justify-between pb-[22px] mb-1.5">
  ...
  <div class="flex items-center gap-3.5">
    <FlowMark />
    <div>
      <div class="font-display text-[27px] leading-none mb-1 tracking-[-0.02em] text-slate-900 dark:text-slate-100">
        flowd
      </div>
  ...
  <div class="flex items-center gap-2.5">
    ...
    <div class="inline-flex bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg p-[3px]">
      {#each views as v}
        <button
          class="border-0 px-3.5 py-1.5 text-[12.5px] rounded-md cursor-pointer font-sans transition-colors duration-100
            {view === v
    ...
    <!-- Theme toggle -->
    <button
      class="w-9 h-9 rounded-lg bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-500 dark:text-slate-400
             inline-grid place-items-center cursor-pointer
             hover:text-slate-700 dark:hover:text-slate-200 hover:border-slate-300 dark:hover:border-slate-500
